...
public class DefaultGraphOfRuleDependenciesViewer {
  ...
  private void addDependencies(GraphOfRuleDependencies g, Graph graphDisp) {
    for (Rule src : g.getRules()) {
      for (Rule dest : g.getTriggeredRules(src)) {
        ...
        Edge e = graphDisp.addEdge(edgeID.toString(), src.getLabel(), dest.getLabel(), true);
    ...
    if (!l.isEmpty()) {
      for (List<Rule> c : l) {
        for (DefaultDirectedLabeledEdge e : ((DefaultLabeledGraphOfRuleDependencies) g)
            .getBadEdges(c)) {
          if (graphDisp.getEdge(e.getFirst() + "|" + e.getSecond()).getAttribute("ui.class")
              .toString().compareTo("moins") == 0)
  ...
  private void computeSccVertices(StronglyConnectedComponentsGraph<Rule> sccGraph, Graph graphDisp,
      GraphOfRuleDependencies graph) {
    /* Vertices */
  ...
  private void addSccDependencies(StronglyConnectedComponentsGraph<Rule> sccGraph, Graph graphDisp,
      GraphOfRuleDependencies graph) {
    /* Edges */
    for (int i = 0; i < sccGraph.getNbrComponents(); i++) {
        ...
        for (int j = 0; j < sccGraph.getNbrComponents(); j++) {
          ...
          for (Rule dest : ((DefaultLabeledGraphOfRuleDependencies) graph).getInhibitedRules(src)) {
            if (s.contains(dest) && graphDisp.getEdge(edgeID) == null && i != j) {
              Edge e = graphDisp.addEdge(edgeID, "C" + i, "C" + j, true);
              e.setAttribute("label", '-');
              e.addAttribute("ui.class", "moins");
            }
          }
